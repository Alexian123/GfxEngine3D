...
Surface createSurface(Material material, vec2 texCoord, vec3 normal, vec3 fragPos, vec3 viewPos)
{
    Surface surface;
    surface.diffuseColor = texture(material.diffuseMap, texCoord).rgb;
    surface.specularColor = texture(material.specularMap, texCoord).rgb;
    surface.emissionColor = texture(material.emissionMap, texCoord).rgb;
    surface.normal = normalize(normal);
    surface.fragPos = fragPos;
    surface.viewDir = normalize(viewPos - fragPos);
